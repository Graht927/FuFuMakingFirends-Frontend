      ...
      <view class="other-login">
        ...
        <view class="social-icons">
          <view class="icon-item">
            <uni-icons type="weixin" size="28" color="#ffffff"/>
          </view>
        </view>
      </view>
    </view>
    <view class="footer">
      <text class="register" @click="toRegister()">还没有账号？立即注册</text>
      <text class="policy">登录即表示同意 《用户协议》 和 《隐私政策》</text>
    </view>
  </view>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    handleGetCode() {
      ...
      // 4. 手机号验证通过，跳转到验证码页面
      uni.navigateTo({
        url: `/pages/PhoneCodePage?phone=${this.phoneNumber}`+"&t=login"
      });
